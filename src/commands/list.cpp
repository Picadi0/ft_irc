...
void IRC::listChannelsss(Client sender ,string channelNames)
{
    ...
    if (channelNames.empty())
        ...
        {
            sendMsg(sender.getSockfd(), ":" + sender.getHostInfo() +" 322 " + sender.getNickname() + " " + channel->getName() + " " + (channel->getMaxClientCount() == 0 ? "99" : intToString(channel->getMaxClientCount())) + "/" + intToString(channel->getClients().size()) + " :" + channel->getTopic());
            channel++;
        }
    else
    {
